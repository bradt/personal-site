 ...
 *
 * PHP's built-in http_ppp_bitly_build_query does the insane thing of converting
 ...
 * @return string
 */
function ppp_bitly_build_query(Array $params) {
    $ret = array();
    $fmt = '%s=%s';
    $separator = '&';
...
class Bitly
{
    ...
    public function getAuthUrl() {
      return 'https://bitly.com/oauth/authorize?clientid=' . bitly_clientid . '&state=ppp_enable_bitly&redirect_uri=' . admin_url( 'admin.php?page=ppp-social-settings' );
    }
    ...
    public function shorten($url, $domain=null)
    {
        ...
        $results = $this->call('v3/shorten', $params, false, false);
     ...
     */
    public function userInfo($login=null, $fullName=null, $json=true)
    {
        $params = array();
        if ($login !== null) {
            $params['login'] = $login;
        ...
        return $this->call('v3/user/info', $params, false, $json );
    }
    ...
    protected function call($endpoint, Array $params=null, $post=false, $json=true)
    {
        ...
        if ($post) {
            ...
            $query = ppp_bitly_build_query($params);
            $url .= '?' . $query;
            curl_setopt($ch, CURLOPT_URL, $url);
        }
        $result = curl_exec($ch);
