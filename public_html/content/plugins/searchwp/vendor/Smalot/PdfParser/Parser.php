...
class Parser
{
    ...
    protected function parseTrailer($structure, $document)
    {
        ...
        foreach ($structure as $name => $values) {
            ...
            if (is_numeric($values)) {
                $trailer[$name] = new \Smalot\PdfParser\Element\ElementNumeric($values, $document);
            } elseif (is_array($values)) {
                $value          = $this->parseTrailer($values, null);
                $trailer[$name] = new \Smalot\PdfParser\Element\ElementArray($value, null);
            } elseif (strpos($values, '_') !== false) {
                $trailer[$name] = new \Smalot\PdfParser\Element\ElementXRef($values, $document);
            } else {
                $trailer[$name] = $this->parseHeaderElement('(', $values, $document);
            }
        }

        return new \Smalot\PdfParser\Header($trailer, $document);
    }
        ...
        foreach ($structure as $position => $part) {
            switch ($part[0]) {
                case '[':
                    ...
                    $header = new \Smalot\PdfParser\Header($elements, $document);
                    break;
                ...
                default:
                    if ($part != 'null') {
                        ...
                        if ($element) {
                            $header = new \Smalot\PdfParser\Header(array($element), $document);
                        }
                    }
                    break;
    ...
    protected function parseHeader($structure, $document)
    {
        ...
        return new \Smalot\PdfParser\Header($elements, $document);
    }
    ...
    protected function parseHeaderElement($type, $value, $document)
    {
        switch ($type) {
            ...
            case 'numeric':
                return new \Smalot\PdfParser\Element\ElementNumeric($value, $document);

            case 'boolean':
                return new \Smalot\PdfParser\Element\ElementBoolean($value, $document);

            case 'null':
                return new \Smalot\PdfParser\Element\ElementNull($value, $document);

            case '(':
                if ($date = \Smalot\PdfParser\Element\ElementDate::parse('(' . $value . ')', $document)) {
                    return $date;
                } else {
                    return \Smalot\PdfParser\Element\ElementString::parse('(' . $value . ')', $document);
                }
            ...
            case 'objref':
                return new \Smalot\PdfParser\Element\ElementXRef($value, $document);
            ...
            case '[':
                ...
                return new \Smalot\PdfParser\Element\ElementArray($values, $document);
